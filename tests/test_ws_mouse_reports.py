...
class WsMouseReportTest(unittest.TestCase):
    ...
    def test_pure_x_delta_updates_third_axis_byte(self) -> None:
        report = self._report_for('{"type":"mouse","dx":42}')
        self.assertEqual(report, [1, 0x00, 42, 0, 0, 0])

    def test_pure_y_delta_updates_fourth_axis_byte(self) -> None:
    ...
    def test_dx_message_keeps_y_zero(self) -> None:
        ...
        self.assertEqual(report[2], 5)
        self.assertEqual(report[3], 0)
    ...
    def test_dx_with_left_click_sets_expected_bytes(self) -> None:
        report = self._report_for(
            '{"type":"mouse","dx":5,"dy":0,"buttons":{"left":true}}'
        )
        self.assertEqual(report, [1, 0x01, 5, 0, 0, 0])
