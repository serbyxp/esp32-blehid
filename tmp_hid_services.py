...
class GATTS:
    NO_ERROR = const(0x00)
    ERROR_INVALID_HANDLE = const(0x01)
    ERROR_READ_NOT_PERMITTED = const(0x02)
    ERROR_WRITE_NOT_PERMITTED = const(0x03)
    ERROR_INSUFFICIENT_AUTHENTICATION = const(0x05)
    ERROR_REQ_NOT_SUPPORTED = const(0x06)
    ERROR_INSUFFICIENT_AUTHORIZATION = const(0x08)
    ERROR_ATTR_NOT_FOUND = const(0x0a)
    ERROR_INSUFFICIENT_ENCRYPTION = const(0x0f)
    ERROR_WRITE_REQ_REJECTED = const(0xFC)
...
class HumanInterfaceDevice(object):
    ...
    def ble_irq(self, event, data):
        if event == IRQ.CENTRAL_CONNECT:                                                                               # Central connected.
        ...
        elif event == IRQ.CENTRAL_DISCONNECT:                                                                          # Central disconnected.
        ...
        elif event == IRQ.GATTS_WRITE:                                                                                 # Write operation from client.
            ...
            if description is None:
                ...
                return GATTS.ERROR_ATTR_NOT_FOUND
            else:
        ...
        elif event == IRQ.GATTS_READ_REQUEST:                                                                          # Read request from client.
            conn_handle, attr_handle = data
            ...
            if conn_handle != self.conn_handle:                                                                         # If different connection, return no permission.
                ...
                return GATTS.ERROR_INSUFFICIENT_AUTHENTICATION
                ...
                return GATTS.ERROR_INSUFFICIENT_ENCRYPTION
            else:                                                                                                       # Otherwise, return no error.
                return GATTS.NO_ERROR
        elif event == IRQ.GATTS_INDICATE_DONE:                                                                         # A sent indication was done. (We don't use indications currently. If needed, define a callback function and override this function.)
            conn_handle, value_handle, status = data
            print("Indicate done:", data)
        elif event == IRQ.MTU_EXCHANGED:                                                                               # MTU was exchanged, set it.
            conn_handle, mtu = data
            self._ble.config(mtu=mtu)
            print("MTU exchanged:", mtu)
        elif event == IRQ.CONNECTION_UPDATE:                                                                           # Connection parameters were updated.
        ...
        elif event == IRQ.ENCRYPTION_UPDATE:                                                                           # Encryption was updated.
        ...
        elif event == IRQ.SET_SECRET:                                                                                  # Set secret for bonding.
            ...
            return True
        elif event == IRQ.GET_SECRET:                                                                                  # Get secret for bonding
    ...
    def start(self):
        if self.device_state is HumanInterfaceDevice.DEVICE_STOPPED:
            ...
            self._ble.active(True)                                                                                      # Turn on BLE radio.
...
class Keyboard(HumanInterfaceDevice):
    ...
    def ble_irq(self, event, data):
        if event == IRQ.GATTS_WRITE:                                                                                   # If a client has written to a characteristic or descriptor.
